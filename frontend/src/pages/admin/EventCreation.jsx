...
import { Download, Upload, FileDown, CheckCircle2, AlertCircle, X } from 'lucide-react';
    ...
    <div style={modalStyle} onClick={onClose}>
      <div style={contentStyle} onClick={(e) => e.stopPropagation()}>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '20px' }}>
          <h3 style={{ margin: 0, fontSize: '20px', fontWeight: 'bold' }}>Bulk Upload Events</h3>
...
const EventCreationPage = () => {
  ...
  const handleDownloadTemplate = async () => {
    try {
      const response = await api.get('/admin/event-creations/download-template', {
        responseType: 'blob'
      });
      const url = window.URL.createObjectURL(new Blob([response.data]));
      const link = document.createElement('a');
      link.href = url;
      link.setAttribute('download', 'event_creation_template.csv');
  ...
  const handleDownloadCSV = async (filtered) => {
    try {
      let endpoint = '/admin/event-creations/download-csv';
      if (filtered) {
        const params = new URLSearchParams();
        if (search) params.append('search', search);
        if (industryFilter) params.append('industry', industryFilter);
      ...
      const link = document.createElement('a');
      link.href = url;
      link.setAttribute('download', 'event_creations_export.csv');
      document.body.appendChild(link);
      link.click();
      link.remove();
  ...
  const handleBulkUpload = async (file) => {
    ...
    try {
      ...
      const response = await api.post('/admin/event-creations/bulk-upload', formData, {
        headers: {
          'Content-Type': 'multipart/form-data'
        }
      });
