...
const RadioOrders = () => {
    ...
    return (
      <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100vh', backgroundColor: theme.backgroundSoft }}>
        <div style={{ textAlign: 'center' }}>
          ...
          <p style={{ color: theme.textSecondary }}>You don't have permission to access radio orders management.</p>
        </div>
      </div>
    );
  }
  ...
  const fetchOrders = async () => {
    ...
    try {
      ...
      const response = await api.get(`/radio-orders?${params}`);
  ...
  const handleStatusChange = async (orderId, newStatus) => {
    ...
    try {
      const response = await api.put(`/radio-orders/${orderId}/status`, { status: newStatus });

      // Show success message with email confirmation
      alert(`✅ Radio order ${newStatus} successfully!\n\n📧 Email notifications have been sent to:\n• The applicant\n• Admin team\n\nThe status has been updated in the system.`);
  ...
  const handleDeleteOrder = async (orderId) => {
    if (!window.confirm('Are you sure you want to delete this radio order?')) return;

    try {
      await api.delete(`/radio-orders/${orderId}`);
      fetchOrders();
    } catch (error) {
          ...
          <div className="flex justify-between items-center py-3">
            <div className="flex items-center">
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                <Icon name="radio" size="lg" style={{ color: '#1976D2' }} />
            ...
            <div style={{ background: '#fff', borderRadius: 12, padding: 28, border: `4px solid #000`, display: 'flex', justifyContent: 'space-between', gap: 16, marginBottom: 24, alignItems: 'center' }}>
              <div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 8 }}>
                  ...
                  <h1 style={{ margin: 0, fontSize: 34, fontWeight: 800 }}>Radio Interview Orders</h1>
                </div>
                      ...
                      <tr key={order.id} style={{
                        ...
                        <td style={{ padding: '16px' }}>
                          <div style={{ display: 'flex', gap: '6px' }}>
                            <button
                              onClick={() => handleStatusChange(order.id, order.status === 'approved' ? 'rejected' : 'approved')}
                              style={{
                                padding: '6px 10px',
                                backgroundColor: order.status === 'approved' ? '#ff9800' : '#4caf50',
                                color: '#fff',
                                border: 'none',
                                borderRadius: '4px',
                                fontSize: '12px',
                              ...
                              title={`${order.status === 'approved' ? 'Reject' : 'Approve'} radio order (sends email notifications)`}
                            >
                              {order.status === 'approved' ? 'Reject' : 'Approve'}
                            </button>
                            <button
                              ...
                              title="Permanently delete this radio order"
                            >
                              Delete
                            </button>
                          </div>
              ...
              {orders.length === 0 && (
                <div style={{ padding: '80px', textAlign: 'center', color: theme.textSecondary }}>
                  ...
                  <div style={{ fontSize: '20px', fontWeight: '600', marginBottom: '8px' }}>
                    No radio orders available
                  </div>
                  <div style={{ fontSize: '16px' }}>
                    No radio interview booking requests have been received yet.
                  </div>
                </div>
              )}
            </div>
