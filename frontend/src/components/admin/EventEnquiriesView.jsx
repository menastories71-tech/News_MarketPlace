...
const EventEnquiriesView = () => {
  const { admin, logout, hasRole, hasAnyRole, getRoleLevel } = useAdminAuth();

  // Check if user has permission to view event enquiries
    ...
    return (
      <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100vh', backgroundColor: theme.backgroundSoft }}>
        <div style={{ textAlign: 'center' }}>
          ...
          <p style={{ color: theme.textSecondary }}>You don't have permission to view event enquiries.</p>
  ...
  const fetchEnquiries = async () => {
    try {
      ...
      const response = await api.get(`/event-enquiries/admin?${params}`);
      ...
      alert('Failed to load event enquiries. Please try again.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const filteredEnquiries = useMemo(() => {
    ...
    if (debouncedSearchTerm) {
      filtered = filtered.filter(enquiry =>
        enquiry.event_name.toLowerCase().includes(debouncedSearchTerm.toLowerCase()) ||
        enquiry.contact_person_name.toLowerCase().includes(debouncedSearchTerm.toLowerCase()) ||
        enquiry.contact_person_email.toLowerCase().includes(debouncedSearchTerm.toLowerCase())
      );
    }
    ...
    if (dateFromFilter) {
      filtered = filtered.filter(enquiry => new Date(enquiry.created_at) >= new Date(dateFromFilter));
    }

    if (dateToFilter) {
      filtered = filtered.filter(enquiry => new Date(enquiry.created_at) <= new Date(dateToFilter));
    }
  ...
  const handleViewEnquiry = async (enquiry) => {
    try {
      ...
      if (enquiry.status === 'new') {
        await api.put(`/event-enquiries/admin/${enquiry.id}/status`, { status: 'viewed' });
        // Update local state
        ...
        <div style={{ display: 'flex', gap: 24, alignItems: 'flex-start' }}>
          <main style={{ flex: 1, minWidth: 0, paddingLeft: !isMobile ? leftGap : 0 }}>
            ...
            <div style={{ background: '#fff', borderRadius: 12, padding: 28, border: `4px solid #000`, display: 'flex', justifyContent: 'space-between', gap: 16, marginBottom: 24, alignItems: 'center' }}>
              <div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 8 }}>
                  ...
                  <h1 style={{ margin: 0, fontSize: 34, fontWeight: 800 }}>Event Enquiries</h1>
                </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 16, marginBottom: 24 }}>
              {[
                { label: 'Total Enquiries', value: stats.total, icon: 'calendar', bg: '#e6f0ff' },
            ...
            <div style={{ backgroundColor: '#fff', padding: '20px', borderRadius: '12px', marginBottom: '16px', boxShadow: '0 8px 20px rgba(2,6,23,0.06)' }}>
              <div style={{ display: 'flex', gap: '16px', alignItems: 'center', flexWrap: 'wrap' }}>
                <div style={{ position: 'relative', flex: 1, minWidth: '300px' }}>
                  ...
                  <input
                    type="text"
                    placeholder="Search enquiries by event name, contact name, email..."
                    value={searchTerm}
                      ...
                      <tr>
                        <td colSpan="6" style={{ padding: '40px', textAlign: 'center', color: theme.textSecondary }}>
                          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '12px' }}>
                            <Icon name="calendar" size="lg" style={{ color: theme.textDisabled }} />
          ...
          <div style={{
            ...
            <div style={{ display: 'grid', gap: '16px' }}>
              <div style={{ backgroundColor: '#f8fafc', padding: '16px', borderRadius: '8px' }}>
                <h3 style={{ margin: '0 0 12px 0', fontSize: '18px', fontWeight: '700', color: theme.textPrimary }}>Event Information</h3>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '12px' }}>
                  <div><strong>Event Name:</strong> {selectedEnquiry.event_name}</div>
                  <div><strong>Event Date:</strong> {selectedEnquiry.event_date ? new Date(selectedEnquiry.event_date).toLocaleDateString() : 'Not specified'}</div>
                  <div><strong>Organiser:</strong> {selectedEnquiry.organiser || 'Not specified'}</div>
                  <div><strong>Event Industry:</strong> {selectedEnquiry.event_industry || 'Not specified'}</div>
                  <div><strong>Event Sub Industry:</strong> {selectedEnquiry.event_sub_industry || 'Not specified'}</div>
                  <div><strong>Location:</strong> {selectedEnquiry.city}, {selectedEnquiry.country}</div>
                  <div><strong>Venue:</strong> {selectedEnquiry.event_venue_name || 'Not specified'}</div>
                  <div><strong>Event Mode:</strong> {selectedEnquiry.event_mode || 'Not specified'}</div>
                  <div><strong>Event Type:</strong> {selectedEnquiry.event_type || 'Not specified'}</div>
                  <div><strong>Organised By:</strong> {selectedEnquiry.event_organised_by || 'Not specified'}</div>
                  <div><strong>Commercial Type:</strong> {selectedEnquiry.event_commercial || 'Not specified'}</div>
                  <div><strong>Entrance:</strong> {selectedEnquiry.event_entrance || 'Not specified'}</div>
                </div>
              </div>

              <div style={{ backgroundColor: '#f8fafc', padding: '16px', borderRadius: '8px' }}>
                <h3 style={{ margin: '0 0 12px 0', fontSize: '18px', fontWeight: '700', color: theme.textPrimary }}>Contact Information</h3>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '12px' }}>
                  <div><strong>Name:</strong> {selectedEnquiry.contact_person_name}</div>
                  <div><strong>Email:</strong> {selectedEnquiry.contact_person_email}</div>
                  <div><strong>Phone:</strong> {selectedEnquiry.contact_person_number || 'Not provided'}</div>
                  <div><strong>WhatsApp:</strong> {selectedEnquiry.contact_person_whatsapp || 'Not provided'}</div>
                </div>
              </div>

              <div style={{ backgroundColor: '#f8fafc', padding: '16px', borderRadius: '8px' }}>
                <h3 style={{ margin: '0 0 12px 0', fontSize: '18px', fontWeight: '700', color: theme.textPrimary }}>Social Media & Website</h3>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '12px' }}>
                  <div><strong>Website:</strong> {selectedEnquiry.event_website ? <a href={selectedEnquiry.event_website} target="_blank" rel="noopener noreferrer">{selectedEnquiry.event_website}</a> : 'Not provided'}</div>
                  <div><strong>Instagram:</strong> {selectedEnquiry.event_ig ? <a href={selectedEnquiry.event_ig} target="_blank" rel="noopener noreferrer">{selectedEnquiry.event_ig}</a> : 'Not provided'}</div>
                  <div><strong>LinkedIn:</strong> {selectedEnquiry.event_linkedin ? <a href={selectedEnquiry.event_linkedin} target="_blank" rel="noopener noreferrer">{selectedEnquiry.event_linkedin}</a> : 'Not provided'}</div>
                  <div><strong>Facebook:</strong> {selectedEnquiry.event_facebook ? <a href={selectedEnquiry.event_facebook} target="_blank" rel="noopener noreferrer">{selectedEnquiry.event_facebook}</a> : 'Not provided'}</div>
                  <div><strong>YouTube:</strong> {selectedEnquiry.event_youtube ? <a href={selectedEnquiry.event_youtube} target="_blank" rel="noopener noreferrer">{selectedEnquiry.event_youtube}</a> : 'Not provided'}</div>
                </div>
              </div>

              <div style={{ backgroundColor: '#f8fafc', padding: '16px', borderRadius: '8px' }}>
                <h3 style={{ margin: '0 0 12px 0', fontSize: '18px', fontWeight: '700', color: theme.textPrimary }}>Partnership Requirements</h3>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '12px' }}>
                  <div><strong>Market Company Name:</strong> {selectedEnquiry.market_company_name ? 'Yes' : 'No'}</div>
                  <div><strong>Provide Booth:</strong> {selectedEnquiry.provide_booth ? 'Yes' : 'No'}</div>
                </div>
              </div>
              ...
              <div style={{ backgroundColor: '#f8fafc', padding: '16px', borderRadius: '8px' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '12px' }}>
                  ...
                  <div><strong>Submitted:</strong> {formatDate(selectedEnquiry.created_at)}</div>
...
export default EventEnquiriesView;